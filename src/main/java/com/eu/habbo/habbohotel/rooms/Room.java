...
public class Room implements Comparable<Room>, ISerialize, Runnable {
                    ...
                    this.roomSpecialTypes.getRollers().forEachValue(roller -> {
                        ...
                        if (allowUsers) {
                            ...
                            for (RoomUnit unit : unitsOnTile) {
                                ...
                                if (unit.getRoomUnitType() == RoomUnitType.USER) {
                                    ...
                                    if (rollingHabbo != null && rollingHabbo.getHabboInfo() != null) {
                                        ...
                                        if (riding != null) {
                                            ...
                                            messages.add(new RoomUnitOnRollerComposer(ridingUnit, roller, ridingUnit.getCurrentLocation(), ridingUnit.getZ(), tile, newZ, room));
                                            isRiding = true;
                                        }
                                    }
                                }
                                ...
                                messages.add(new RoomUnitOnRollerComposer(unit, roller, unit.getCurrentLocation(), unit.getZ() + (isRiding ? 1 : 0), tile, newZ + (isRiding ? 1 : 0), room));
    ...
    public boolean removeFromQueue(Habbo habbo) {
        try {
            this.sendComposer(new FlatAccessibleMessageComposer(this.getId(), habbo.getHabboInfo().getUsername()).compose());
    ...
    public void refreshRightsForHabbo(Habbo habbo) {
        ...
        if (habbo.hasPermission(Permission.ACC_ANYROOMOWNER)) {
            habbo.getClient().sendResponse(new YouAreOwnerMessageComposer(this.getId()));
        ...
        habbo.getClient().sendResponse(new YouAreControllerMessageComposer(this.getId(), flatCtrl));
